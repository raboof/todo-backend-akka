...
trait TodoRoutes extends TodoMarshalling
  ...
  val x: WrappedDirective[Tuple1[akka.http.javadsl.model.Uri]] = (respondWithHeaders(
    `Access-Control-Allow-Origin`.`*`,
    `Access-Control-Allow-Headers`("Accept", "Content-Type"),
    `Access-Control-Allow-Methods`(GET, HEAD, POST, DELETE, OPTIONS, PUT, PATCH)
  ) & extract(_.request.getUri()))

  def f(uri: akka.http.javadsl.model.Uri): Route = {
  ...
  println(wrappedRoutes)
  val routes: RequestContext => Future[RouteResult] = {
    wrappedRoutes.route
  }
}
